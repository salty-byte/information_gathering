...
import { RSS2App } from '../../src/apps/rss2App';
import * as upload from '../../src/upload';

describe('RSS2App test', () => {
  const APP_NAME = 'RSS2.0';
  ...
  describe('create test', () => {
    beforeAll(() => {
      PropertiesService.getScriptProperties = jest
        ...
        .mockImplementation(() => {
          return {
            getProperty: () =>
              'https://example.com/rss2-1/,https://example.com/rss2-2/',
          };
        });
    });
    ...
    test('fetch and create infos', async () => {
      ...
      expect(UrlFetchApp.fetch).toHaveBeenCalledWith(
        'https://example.com/rss2-1/'
      );

      expect(UrlFetchApp.fetch).toHaveBeenCalledWith(
        'https://example.com/rss2-2/'
      );
    ...
    test('fetch and create infos without duplicate title', async () => {
      ...
      sheet.getRange(1, 3).setValue('https://example.com/rss2-1/1');
      sheet.getRange(2, 1).setValue('Sat, 26 Dec 2020 00:00:00 +0900');
      sheet.getRange(2, 2).setValue('新タイトル');
      sheet.getRange(2, 3).setValue('https://example.com/rss2-1/new');
  ...
  describe('upload test', () => {
    beforeAll(() => {
      PropertiesService.getScriptProperties = jest
        ...
        .mockImplementation(() => {
          return {
            getProperty: () => 'https://example.com/rss2-1/',
          };
        });
    });
